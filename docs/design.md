...
First public release. No behavioural change from 0.8.0: the controller, the popup, the site list, the message contract and the `zoom:<origin>` data model are all unchanged. What is new is everything around the code — a reproducible packaging script and its regression test, MIT licensing, a published privacy policy, store listing assets, and the first real-browser verification pass. The short description in both locales was rewritten to lead with the ultrawide problem rather than with a list of services.
